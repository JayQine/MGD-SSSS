...
with Engine(custom_parser=parser) as engine:
    ...
    criterion_sup = ProbOhemCrossEntropy2d(ignore_label=255, thresh=0.7,
    ...
    for epoch in range(engine.state.epoch, config.nepochs):
        ...
        for idx in pbar:
            ...
            loss_sup = criterion_sup(sup_pred_l, gts)
            dist.all_reduce(loss_sup, dist.ReduceOp.SUM)
            loss_sup = loss_sup / engine.world_size

            loss_sup_r = criterion_sup(sup_pred_r, gts)
            dist.all_reduce(loss_sup_r, dist.ReduceOp.SUM)
            loss_sup_r = loss_sup_r / engine.world_size

            loss_sup_3 = criterion_sup(sup_pred_3, gts)
        ...
        if (epoch > config.nepochs - 80) and (epoch % config.snapshot_iter == 0) or (epoch == config.nepochs - 1):
            if engine.distributed and (engine.local_rank == 0):
