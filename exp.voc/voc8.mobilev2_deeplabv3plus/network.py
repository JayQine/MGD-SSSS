...
from base_model import resnet50, resnet18, resnet101
from mobilenetv2 import mobilenet_v2
from wide_resnet import wide_resnet34_2

class Network(nn.Module):
    def __init__(self, num_classes, criterion, norm_layer, pretrained_model=None):
        super(Network, self).__init__()
        # student
        self.branch1 = SingleSmallNetwork(num_classes, criterion, norm_layer, pretrained_model)
        # teacher1
        self.branch2 = SingleWideNetwork(num_classes, criterion, norm_layer, pretrained_model)
        # teacher2
        self.branch3 = SingleLargeNetwork(num_classes, criterion, norm_layer, pretrained_model)
...
class SingleWideNetwork(nn.Module):
    def __init__(self, num_classes, criterion, norm_layer, pretrained_model=None):
        super(SingleWideNetwork, self).__init__()

        self.backbone = wide_resnet34_2(pretrained_model, norm_layer=norm_layer)
        self.dilate = 2
    ...
    def forward(self, data):
        ...
        if self.training:
            return blocks, v3plus_feature, pred
        return pred
...
class SingleLargeNetwork(nn.Module):
    def __init__(self, num_classes, criterion, norm_layer, pretrained_model=None):
        super(SingleLargeNetwork, self).__init__()
...
class SingleSmallNetwork(nn.Module):
    def __init__(self, num_classes, criterion, norm_layer, pretrained_model=None):
        super(SingleSmallNetwork, self).__init__()

        self.backbone = mobilenet_v2(pretrained=True)

        self.dilate = 2
        for m in self.backbone.features[15:18]:
            m.apply(partial(self._nostride_dilate, dilate=self.dilate))
            self.dilate *= 2

        self.head = HeadSmall(num_classes, norm_layer, config.bn_momentum)
        self.business_layer = []
        self.business_layer.append(self.head)
        self.criterion = criterion
...
class HeadSmall(nn.Module):
    def __init__(self, classify_classes, norm_act=nn.BatchNorm2d, bn_momentum=0.0003):
        ...
        self.reduce = nn.Sequential(
            ...
            norm_act(24, momentum=bn_momentum),
            nn.ReLU(),
        )
        self.last_conv = nn.Sequential(nn.Conv2d(280, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       norm_act(256, momentum=bn_momentum),
                                       nn.ReLU(),
                                       nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       norm_act(256, momentum=bn_momentum),
...
if __name__ == '__main__':
    ...
    out = model(left, step=2)
    print(out[0].shape)

    model_child = model.branch1.backbone


    input = torch.randn(1, 3, 512, 512)
    flops, params = profile(model, inputs=(input,))
    
    macs, params_2 = get_model_complexity_info(model, (3, 512, 512), as_strings=True,
