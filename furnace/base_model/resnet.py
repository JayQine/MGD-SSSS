...
def resnet18(pretrained_model=None, **kwargs):
    ...
    model_dict = res18.state_dict()

    resnet18_ti = timm.create_model("resnet18d", pretrained=True)
    pre_dict = resnet18_ti.state_dict()
    # update state
    model_dict_ori = model.state_dict()
    state_dict = {k:v for k, v in pre_dict.items() if k in model_dict_ori.keys()}
    new_dict = {}
    for k, v in state_dict.items():
        if k == "layer2.0.downsample.1.weight" or k == "layer3.0.downsample.1.weight" or k == "layer4.0.downsample.1.weight":
            continue
        new_dict[k] = v
    model_dict_ori.update(new_dict)
